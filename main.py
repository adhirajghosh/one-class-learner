...
def train_model(model, train_loader, test_loader, train_loader_1, device, args):
    ...
    for epoch in range(args.epochs):
        running_loss = run_epoch(model, train_loader_1, optimizer, center, device, loss)
...
def run_epoch(model, train_loader, optimizer, center, device, loss_fn):
    ...
    #     loss_val = loss_fn(out_1, labels) + center_loss
    #
    #     loss_val.backward()
    #
    #     optimizer.step()
    #
    #     total_num += img1.size(0)
    #     total_loss += loss_val.item() * img1.size(0)
...
def main(args):
    wandb.init(project="DL-AUCROC", entity="adhirajghosh")
    ...
    for i in range(10):
        model = utils.Model(args.model)
        model = nn.DataParallel(model)
        model = model.to(device)
        print('Dataset: {}, Normal Label: {}, LR: {}'.format(args.dataset, i, args.lr))
        train_loader, test_loader, train_loader_1 = utils.get_loaders(dataset=args.dataset, label_class=i, batch_size=args.batch_size)
